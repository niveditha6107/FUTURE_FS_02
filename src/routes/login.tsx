...
function LoginPage() {
  ...
  return (
    <div className="relative flex min-h-screen items-center justify-center overflow-hidden bg-background px-4 py-12">
      <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_20%_20%,oklch(0.7_0.18_265/0.18),transparent_50%),radial-gradient(circle_at_80%_80%,oklch(0.65_0.17_155/0.15),transparent_50%)]" />
      <div className="relative grid w-full max-w-5xl gap-8 lg:grid-cols-2">
        <div className="hidden flex-col justify-between rounded-2xl bg-gradient-to-br from-primary to-primary/70 p-10 text-primary-foreground lg:flex">
          <div className="flex items-center gap-2">
            <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-white/15 backdrop-blur">
              <Sparkles className="h-5 w-5" />
            </div>
            <div>
              <p className="text-lg font-semibold">LeadFlow</p>
              <p className="text-xs uppercase tracking-wider opacity-80">Mini CRM</p>
            </div>
          </div>
          <div>
            <h2 className="text-3xl font-bold leading-tight">Turn contact-form replies into closed deals.</h2>
            <p className="mt-4 text-sm opacity-90">
              Capture, track, and convert every client lead — built for agencies, freelancers, and startups.
            </p>
            <ul className="mt-6 space-y-2 text-sm">
              {["Centralized lead inbox", "Follow-up reminders", "Source & conversion analytics"].map((f) => (
                <li key={f} className="flex items-center gap-2 opacity-90">
                  <span className="h-1.5 w-1.5 rounded-full bg-white/80" /> {f}
                </li>
              ))}
            </ul>
          </div>
          <p className="text-xs opacity-70">Developed by Niveditha Arige · Future Interns</p>
        </div>
        ...
        <Card className="p-8 shadow-xl">
          <div className="mb-6 flex items-center gap-2 lg:hidden">
            ...
            <p className="text-lg font-semibold">LeadFlow CRM</p>
          </div>
